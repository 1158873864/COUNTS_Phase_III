...
function showPicture(imgTag,taskID){
    ...
    testSetName=taskID;     //保存测试集任务名到全局变量testSetName
    ...
    /***要删掉啊啊啊啊啊啊***/
    /*attributeArray = new Array("颜色");    //属性数组
    alternativeArray =new Array("红色,白色,黑色,蓝色");   //备选项数组
    tagMethod=1;*/
        ...
        for(var i=0; i<len; i++){
            ...
            if(p.getElementsByClassName('image')[0].src===modalImg.src){
                ...
                for(var i=0;i<pointList1.length;i++){
                    ...
                    /*
                     * 把这里的保存方法替换成保存测试集中当前图片的标准答案的方法
                     * 任务名为testSetName*/

                    $.ajax(
                        {
                            url:"http://localhost:8080/bosssaveTestSet",
                            data:{url:src,taskName:testSetName,list1:pointList1,list2:pointList2,description:description},
                            async: false,
                                success:function(data){
